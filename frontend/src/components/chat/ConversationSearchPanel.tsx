...
const ConversationSearchPanel = ({
  ...
  const results = useMemo(() => {
    ...
    return sortedMessages.filter((message) => {
      ...
      if (senderFilter !== "all" && message.senderId !== senderFilter) {
        return false;
      }
  ...
  return (
    <aside className="w-[360px] shrink-0 border-l bg-background/95 backdrop-blur-sm">
        ...
        <div className="border-b px-4 py-4">
          <div className="flex items-start justify-between gap-3">
            ...
            <button
              type="button"
              onClick={onClose}
              className="inline-flex size-8 items-center justify-center rounded-full border border-border/70 text-muted-foreground transition hover:bg-accent hover:text-foreground"
            >
              <X className="size-4" />
            </button>
          </div>
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            <label className="flex min-w-0 items-center gap-2 rounded-xl border border-border/70 bg-muted/20 px-3 py-2">
          ...
          {!query.trim() ? (
            ...
            <div className="space-y-2">
              ...
              {results.map((message) => {
                ...
                const sender = conversation.participants.find((participant) => participant._id === message.senderId);
